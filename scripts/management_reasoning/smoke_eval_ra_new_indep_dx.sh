#!/usr/bin/env bash
# Flash-Lite (diagnosis JSON field) on ra_new independent MR arms.
#
# Usage:
#   bash ./scripts/management_reasoning/smoke_eval_ra_new_indep_dx.sh
...
SUITE="${SUITE:-indep_dx_ra_new_smoke_n10}"
if [[ -z "${END_IDX+x}" ]]; then
  if [[ "$SUITE" == *smoke* ]]; then
    END_IDX=9
  else
...
for t, a in resolve_jobs(None, None, suite="indep_dx_ra_new"):
    print(f"{t} {a}")
PY
)
...
echo "Done smoke_eval_ra_new_indep_dx suite=$SUITE"
