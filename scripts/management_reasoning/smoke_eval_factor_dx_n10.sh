#!/usr/bin/env bash
# Flash-Lite (full_response) on factor-pair legacy free-form diagnosis arms.
#
# Usage:
#   bash ./scripts/management_reasoning/smoke_eval_factor_dx_n10.sh
#   SUITE=legacy_dx_factor bash ./scripts/management_reasoning/smoke_eval_factor_dx_n10.sh   # full n
#   END_IDX=99 SUITE=legacy_dx_factor bash ./scripts/management_reasoning/smoke_eval_factor_dx_n10.sh
set -euo pipefail
...
SUITE="${SUITE:-legacy_dx_factor_smoke_n10}"
...
for t, a in resolve_jobs(None, None, suite="legacy_dx_factor"):
    print(f"{t} {a}")
PY
)
...
echo "Done smoke_eval_factor_dx suite=$SUITE"
