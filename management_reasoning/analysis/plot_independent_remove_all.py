#!/usr/bin/env python3
"""Flip/acuity plots: independent raw vs remove_all (per model).

Claude reuses prior ``raw_independent_batch``; Gemini uses both arms from
``independent_remove_all_batch``.

Usage:
  python -m management_reasoning.analysis.plot_independent_remove_all
  python -m management_reasoning.analysis.plot_independent_remove_all --model gemini
"""
...
from management_reasoning.analysis.load_responses import load_independent_remove_all_grid
...
ABLATIONS = ("remove_all",)
ABLATION_DISPLAY = {"remove_all": "Remove All (indep)"}

# Monkey-patch display used by plot_changes helpers via local wrappers if needed.
# plot_changes uses ABLATION_DISPLAY from its module for tick labels — pass keys
# that we also register by temporarily extending that map in run().
...
def run_model(
    *,
    ...
    pc.COLORS.setdefault("remove_all", "#d62728")

    model_dir = MODEL_DIRS[model_key]
    reuse = "independent_batch" if model_key == "claude" else None
    print(f"Loading independent remove_all grid for {model_dir} …")
    grid = load_independent_remove_all_grid(
    ...
    if "remove_all" not in grid or not grid["remove_all"]:
        raise SystemExit(f"Missing remove_all independent for {model_dir}")
    ...
    stem = f"indep_remove_all_{model_key}"
...
def main(argv=None) -> None:
    ...
    for m in models:
        run_model(
            ...
            out_csv=os.path.join(args.csv_dir, f"indep_remove_all_{m}_flips.csv"),
        )
