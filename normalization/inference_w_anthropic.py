...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default="gpt-4.1-mini",
                        help="Model name, e.g. gpt-4o, gpt-4o-mini, gpt-5.1, claude-3-5-sonnet, etc.")

    parser.add_argument("--provider", type=str, default=None, choices=["openai", "anthropic"],
                        help="Explicit API provider (openai/anthropic). If not specified, auto-detects from model name.")
...
def _get_provider(args) -> str:
    """Get provider from args, using auto-detection if not explicitly set."""
    if args.provider is not None:
        return args.provider
    return _detect_provider(args.model)
...
def _query_anthropic(
    ...
    if max_tokens is None:
        max_tokens = 4096  # Default max tokens for Anthropic
...
def query_model(
    ...
    """Query model using the appropriate provider."""
...
def main():
    ...
    if provider == "anthropic":
        ...
        if not api_key:
            raise ValueError("❌ Anthropic API key required. Set --anthropic_api_key or ANTHROPIC_API_KEY environment variable.")
